...
class RarityCalculationService {
  ...
  public async calculateRarityScores(): Promise<void> {
    ...
    for (const collection of collections) {
      console.log(`Calculating rarity for collection: ${collection.collectionName}`);
      ...
      const bulkOps = results.map(result => ({
        updateOne: {
          filter: { tokenId: result.nfts.tokenId, collectionId: collection.collectionId },
          update: { 
            $set: { 
              rarityScore: result.nfts.rarityScore,
              rarityRank: result.nfts.rank,
      ...
      console.log(`Rarity calculation completed for collection: ${collection.collectionName}`);
    }
