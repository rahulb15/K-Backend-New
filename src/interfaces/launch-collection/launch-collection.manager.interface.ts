...
export interface ILaunchCollectionManager {
  ...
  update(collectionName: string, collection: ILaunchCollection): Promise<ILaunchCollection>;
  getByUserId(userId: string): Promise<ILaunchCollection>;
  getAll(page: number, limit: number, search: string): Promise<ILaunchCollection[]>;
