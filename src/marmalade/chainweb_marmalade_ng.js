...
const {createClient, Pact,} = require('@kadena/client')
...
class MarmaladeNGClientSingleton {
  constructor() {
    if (!_instance) {
      _instance = this;
      this.client = null;
  ...
  getClient() {
    // console.log('Getting client:', this.client);
    return this.client;
  }
}

const clientInstance = new MarmaladeNGClientSingleton();

function set_client(new_client) {
  clientInstance.setClient(new_client);
}
...
function set_client_from_data(data) {
  const newClient = new MarmaladeNGClient(data.name, data.node, data.network, data.chain, data.ns, data.bridge_ns);
